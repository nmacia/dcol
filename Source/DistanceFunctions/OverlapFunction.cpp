...
\brief This file contains the implementation of the OverlapFunction methods.
...
OverlapFunction::OverlapFunction () {
    if ( Utils::doScreenStatistics ) 
        std::cout << "    > Creating an Overlap distance function object " << std::endl;
} // end OverlapFunction
...
float OverlapFunction::computeDistance ( float att1, float att2 ) {
    return ( att1 == att2 ? 0 : 1 );
} // end computeDistance
